...
const ShortQuestionsPage = () => {
  ...
  const startExamAttempt = async () => {
    ...
    try {
      ...
      if (responseData.attempt.id) {
        localStorage.setItem("attempt_id", responseData.attempt.id);
        console.log("Saved attempt_id:", responseData.attempt.id);
      } else {
        console.warn("No attempt_id received in response");
      }
  ...
  const submitAllAnswers = async () => {
    ...
    try {
      ...
      console.log("Complete exam attempt response:", completeResponseData);

      // Redirect to results page after successful completion
      if (completeResponse.ok) {
        window.location.href = `./Result`; // Adjust the URL as needed
      }
